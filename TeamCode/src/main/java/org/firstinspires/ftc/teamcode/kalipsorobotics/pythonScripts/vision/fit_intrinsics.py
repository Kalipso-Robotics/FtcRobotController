...
USAGE
    python fit_intrinsics.py RaytracingGroundTruth_*.csv --mount-angle 24
    python fit_intrinsics.py holdout.csv --evaluate

    --mount-angle DEG      pin the mount angle to a physically measured value and
                           fit only (cy, fy). THIS IS THE ONE YOU WANT.
    --mount-angle-search   sweep mountAngle maximising fit linearity. DEGENERATE
                           on floor-only data -- see the warning below. Kept only
...
WHY --mount-angle-search CANNOT BE TRUSTED HERE
    ...
    radians. When every sample lies on ONE plane (the floor), nothing in the data
    distinguishes the two, so the sweep has no minimum to find -- it just runs to
    the edge of its range. On the 2026-09-08 CSV it reports theta=45.6 deg at
    R^2=0.99741, but R^2 climbs monotonically to 50 deg with an identical residual
    spread (21.1mm at 45.6 deg vs 21.3mm at 50 deg vs 27.7mm at 28 deg), and the
    "fitted" cy comes out at 368.9 in a 480-tall image.

    Measure the angle physically and pass --mount-angle. Pinning theta makes the
    remaining (cy, fy) fit linear and well conditioned. To actually SOLVE theta you
    need samples on a second plane (e.g. the ball on a known-height box), which
    breaks the degeneracy; only then is the search meaningful.
...
def load(path, cam_z=None, cam_h=None):
    """
    Read the collector CSV. cam_z / cam_h override the CamOffsetZMM / CamHeightMM
    columns.

    The override matters: those columns record what the COLLECTOR was configured with,
    which is not necessarily what CameraIntrinsics projects with at runtime. The
    2026-09-08 run logged the pre-tilt Z of 151.868 while ARDUCAM ships the
    tilt-corrected 163.470, and fitting against the wrong one shifts fy by ~19 px.
    Fit against the geometry the robot actually uses.
    """
    rows = []
...
def report_fit(rows, args):
    theta_deg = 0.0
    if args.mount_angle is not None:
        theta_deg = args.mount_angle
        print("MOUNT ANGLE PINNED at %+.2f deg (positive = lens points DOWN)."
              % theta_deg)
        print("  Fitting only (cy, fy), which is linear and well conditioned.")
        print()
    elif args.mount_angle_search:
        ...
        print()
        print("  *** WARNING: theta and cy are DEGENERATE on single-plane data. ***")
        print("  Shifting cy by d px is the same ray as tilting by d/fy rad, so with")
        print("  every sample on the floor this sweep has no real minimum -- it runs to")
        print("  the edge of its range and reports a cy that need not be physical.")
        print("  Sanity-check R^2 at nearby angles: if it is still climbing, this")
        print("  number is meaningless. Measure the angle and use --mount-angle DEG.")
        print()
    ...
    # --- paste-ready --------------------------------------------------------
    # cx/fx are only identifiable if the ball was actually moved off-centre. With every
    # row centred, the horizontal fit's sole lever arm is the camera's own lateral
    # offset, so it is really just re-measuring CAM_OFFSET_X_MM and its "fx" is
    # whatever makes that offset consistent. It still reports a tight R^2, which is
    # exactly what makes it dangerous to paste. Refuse to emit it.
    n_offcentre = sum(1 for r in rows
                      if abs(r["known_lat"]) > CENTERED_LATERAL_MM)
    if n_offcentre < 3:
        fx_final, cx_final = shipped_fx, shipped_cx
        print("!! cx/fx NOT SOLVED: only %d off-centre rows (need >= 3)."
              % n_offcentre)
        print("   Every sample is centred, so the horizontal fit has no lever arm but")
        print("   the camera's own lateral offset -- it re-measures CAM_OFFSET_X_MM,")
        print("   not fx. Keeping the shipped fx/cx below. To solve them, collect ~6")
        print("   bursts with the ball off to one side at 2-3 distances and re-run.")
        print()
    else:
        fx_final = None
...
def selftest():
    ...
    print()
    print("theta/cy degeneracy: single-plane data cannot separate them")
    # Same synthetic floor samples, fitted at three DIFFERENT assumed angles. If the
    # sweep were identifiable these R^2 values would differ sharply and the true 18 deg
    # would win outright. They do not, which is exactly why --mount-angle-search is
    # untrustworthy and --mount-angle exists. Assert the flatness so nobody "fixes"
    # the search back into the recommended path.
    rows = synth(18.0)
    r2s = []
    for deg in (14.0, 18.0, 22.0):
        v = fit_vertical(rows, math.radians(deg))
        r2s.append(v["r2"])
        print("  theta=%5.1f  R^2=%.6f  fy=%8.3f  cy=%8.3f"
              % (deg, v["r2"], v["fy"], v["cy"]))
    spread = max(r2s) - min(r2s)
    ok = spread < 1e-3
    print("  %-34s %12.2e  want %12s  %s"
          % ("R^2 spread over +/-4 deg", spread, "< 1.0e-03", "ok" if ok else "FAIL"))
    if not ok:
...
def main():
    ...
    parser.add_argument("--cam-z", type=float, default=None, metavar="MM",
                        help="override the CamOffsetZMM column with the value "
                             "CameraIntrinsics actually projects with")
    parser.add_argument("--cam-height", type=float, default=None, metavar="MM",
                        help="override the CamHeightMM column")
    parser.add_argument("--mount-angle", type=float, default=None, metavar="DEG",
                        help="pin the mount angle (positive = lens down) and fit "
                             "only cy/fy. Preferred: measure it physically.")
    parser.add_argument("--mount-angle-search", action="store_true",
                        help="sweep mountAngle for best linearity. DEGENERATE on "
                             "floor-only data (theta trades off against cy); only "
    ...
    rows = load(args.csv, cam_z=args.cam_z, cam_h=args.cam_height)
    if not rows:
