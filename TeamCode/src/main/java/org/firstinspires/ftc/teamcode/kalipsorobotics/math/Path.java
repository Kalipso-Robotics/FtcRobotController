...
//
    ...
    public Optional<Position> searchFrom(Position currentPosition, double radius, int startSegmentIndex) {
        int start = Math.max(0, Math.min(startSegmentIndex, numSegments() - 1));

        for (int i = start; i < numSegments(); i++) {
            Segment segment = getSegment(i);
