...
/**
 * AutoBasketFunnel with integrated stuck detection and recovery
 * Extends the original AutoBasketFunnel and adds CheckStuckRobot functionality
 */
@Autonomous(name = "AutoBasket with Stuck Detection")
public class AutoBasketFunnelWithCheckStuck extends LinearOpMode {
...
//        redAutoBasket.addAction(intake3);
        ...
        OdometryFileWriter odometryFileWriter = new OdometryFileWriter("AutoBasketWithCheckStuck", opModeUtilities);
        ...
        while (opModeIsActive()) {
            ...
            if (currentPosition != null) {
                ...
                if (isStuck && !isCurrentlyStuck) {
                    ...
                    Log.d("AutoBasketFunnelWithCheckStuck", "Robot stuck detected - pausing auto sequence");
                    telemetry.addLine("⚠️ ROBOT STUCK - Unstucking in progress...");
                    telemetry.update();
                } else if (!isStuck && isCurrentlyStuck) {
                    isCurrentlyStuck = false;
                    Log.d("AutoBasketFunnelWithCheckStuck", "Robot unstuck - resuming auto sequence");
                    telemetry.addLine("✅ Robot unstuck - continuing auto");
                    telemetry.update();
                }

                // Only update actions if not currently stuck
                if (!isCurrentlyStuck) {
