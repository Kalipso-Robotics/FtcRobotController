...
//        double bRightPower = powerX + powerY - powerAngle;
    ...
    public double[] calculatePower(Gamepad gamepad) {
        //negative because gamepad y is flip
        double forward = 0.5 * -gamepad.left_stick_y * -gamepad.left_stick_y * -gamepad.left_stick_y; //cube so slower is slower and faster is faster
        double turn = 0.5 * gamepad.right_stick_x * gamepad.right_stick_x * gamepad.right_stick_x;
        double strafe = 0.5 * gamepad.left_stick_x * gamepad.left_stick_x * gamepad.left_stick_x;
